...
1. SSH to our production machine on apps.library.caltech.edu
...
6. Run `sudo make compile`
7. Run `make test`
8. If tests pass then restart the systemd services otherwise debug issues

## Notes

- Use `make compile`, not plain `make`. The default target also regenerates the
  documentation with CMTools, which is not installed on production, and rebuilds
  the browser bundles in `htdocs/modules/` — those are committed to the
  repository and should come from `git pull`, not from a build on the server.

- `make compile` needs `sudo` because the files in `/Sites/content-dashboard`
  are owned by the account that first deployed them.
